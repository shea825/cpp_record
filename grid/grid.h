...
#ifndef MAINPROJ_GRID_H
#define MAINPROJ_GRID_H
...
public:
     ...
     * ---------------------------------
     * Reinitialize the grid to have the specified number of rows
...
public:
    ...
    public:
        ...
        const iterator operator ++(int) {//add 'const' to avoid i++++ legal
            iterator copy(*this);
            operator++();
            return copy;
        }
...
//    friend class GridRowConst;//no need
};
...
Grid<ValueType>::Grid(std::initializer_list<std::initializer_list<ValueType> > list)
    ...
    for (int row = 0; row < nRows; row++) {
        if (rowItr->size() != nCols) {
            perror("Grid::constructor: initializer list is not rectangular (must have same # cols in each row)");
        }
...
const T& randomElement(const Grid<T>& grid) {
    if (grid.isEmpty()) {
        perror("randomElement: empty grid was passed");
    }
